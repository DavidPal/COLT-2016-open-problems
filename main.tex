...
We will use the following well know facts.  Negative Shannon entropy, $-H(p)$,
...
Second, Hedge and the bound assume that $\ell_1, \ell_2, \dots, \ell_T \in
[-1,1]^N$. We would like to allow loss vectors that are arbitrary vectors in
$\R^N$.

Hedge is equivalent to FTRL\footnote{Hedge is
...
satisfying~\eqref{equation:parameter-free-bound-experts} satisfies
...
Orabona-Pal-2016-parameter-free} on algorithms that satisfy a
slightly looser\footnote{Earlier papers have extra logarithmic factors. Last two papers have only the extra $1+$ and fixed multiplicative constant hidden on $\widetilde O(\cdot)$}
versions of~\eqref{equation:parameter-free-bound-experts}
\begin{equation}
...
FTRL with regularizer $\frac{1}{2}\norm{u}^2$ and learning rate $\eta$ satisfies
\begin{equation}
...
\end{equation}
With a learning rate $\eta = D/\sqrt{T}$, we get a family of algorithms
parametrized by $D \in [0,\infty)$ that satisfy
\begin{equation}
...
\eqref{equation:ftrl-vanila-3}, since \eqref{equation:olo-parameter-free} holds
for a single algorithm, whereas \eqref{equation:ftrl-vanila-3} holds for a
family of algorithms parametrized by $D$. Finally, note that
\eqref{equation:olo-parameter-free} has better dependency on $\norm{u}$
...
McMahan-Orabona-2014} that satisfy a slightly weaker version of \eqref{equation:olo-parameter-free}
\begin{equation}
\label{equation:olo-parameter-free-2}
\forall u \in \H \qquad \Regret_T(u) \le \widetilde O(\sqrt{\norm{u} T}) \; .
\end{equation}
The $\widetilde O$ notation hides poly-logarithmic factors\footnote{It can be
shown that for OLO over Hilbert space extra poly-logarithmic factor is necessary.} in
$\norm{u}$ and $T$.  Algorithms satisfying \eqref{equation:olo-parameter-free-2}
...
\cite{Orabona-Pal-2015} showed that the resulting algorithm is scale-free and
for any sequence of loss vectors $\{\ell_t\}_{t=1}^\infty$, $\ell_t \in \H$, it satisfies
$$
...
of parameter-free and scale-free algorithms for OLO over $\H$. Formally:
\emph{Is there an algorithm (without any tuning parameters) such that for any sequence of loss
vectors $\{\ell_t\}_{t=1}^\infty$, $\ell_t \in \H$,}
$$
...
are unlikely to satisfy the bound.
