...
\cite{Foster-Rakhlin-Sridharan-2015, Koolen-van-Erven-Grunwald-2014}.
...
the maximum norm $\norm{\ell_t}$ seen so far. This approach fails the way as
for LEA. A more promising approach is to use infinitely many copies of FTRL
indexed by $i=1,2,\dots$, with scale-free learning rates
$C_i/\sqrt{\sum_{j=1}^{t-1} \norm{\ell_j}^2}$ for suitable constants $C_i$ and
combine their predictions using AdaHedge with non-uniform prior.
