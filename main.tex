...
$\ell_t \in \H$,}
$$
\forall T \ge 0 \ \ \forall u \in \H \ \ \Regret_T(u) \le \polylog(1 + \norm{u}) \norm{u} \sqrt{T} \max_{1 \le t \le T} \norm{\ell_t} + O(\sqrt{T}) \max_{1 \le t \le T} \norm{\ell_t}.
$$
...
$\log(1+\norm{u})$ and $O(\sqrt{T})$ represents a function
bounded by a constant multiple of $\sqrt{T}$.
